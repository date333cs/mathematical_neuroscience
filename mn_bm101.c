...
void project32(){
    ...
    for (j=0; j<n_iter; j++) {  // この課題の場合は，f_ij は変化しないので，この for 文は *** に置いてもよい．
      for (i=0; i<n_sub_iter; i++) {
	// generate x according to q(x) (true distr.) .
      }
      // compute f_ij
      ...
      set_x_zero();  //  except x[0]=1;
      for (i=0; i<n_sub_iter; i++) {
	// choose a unit randomly. update its value.
      }
      // compute g_ij
      
      // learning_w(); // update weight connections
      // compute D(q,p);
